...
export type SuperProctorTab = "assign-proctor" | "manage-proctors";

type Props = {
  open: boolean;
  setOpen: (v: boolean) => void;
  activeTab: SuperProctorTab;
  setActiveTab: (t: SuperProctorTab) => void;
};
...
const NAV_ITEMS: {
  ...
  {
    tab:   "assign-proctor",
    label: "Assign Proctors",
    icon:  <UserCheck size={17} className="shrink-0" />,
  },
  {
    tab:   "manage-proctors",
...
export default function SuperProctorSidebar({
  open, setOpen, activeTab, setActiveTab,
}: Props) {
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-100 px-3 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-700">Super Proctor</span>
          </div>
          <button
            onClick={() => setOpen(false)}
            className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition"
